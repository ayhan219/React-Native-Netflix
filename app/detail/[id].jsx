  ...
  const getSpecificData = async () => {
    try {
      const response = await axios.get(
        `https://api.themoviedb.org/3/movie/${singleData.id}?api_key=${key.APIKEY}&language=en-US`
      );
      setDetailData(response.data);
    } catch (error) {
      try {
        const response = await axios.get(
          `https://api.themoviedb.org/3/tv/${singleData.id}?api_key=${key.APIKEY}&language=en-US`
        );
        setDetailData(response.data);
      } catch (error) {
        return null;
    ...
    <ScrollView className="w-full h-full bg-black relative">
      ...
      <View className="p-4">
        ...
        <Text className="text-gray-400 text-lg mt-2">
          ...
          {singleData.vote_average.toFixed(1)} {"  |  "} ⏳ {detailData.runtime}{" "}
          min | {"  |  "}
      ...
      <View className="items-center">
        <View className="flex-row flex-grow gap-2">
        {
          detailData.genres?.map((item)=>(
            <View key={item.id} className="w-24 h-10 border border-gray-500 flex items-center justify-center rounded-md">
      ...
      <View className="w-full items-center">
        <TouchableOpacity
          ...
          onPress={() =>
            Linking.openURL(`https://www.imdb.com/title/${detailData.imdb_id}`)
          }
        >
